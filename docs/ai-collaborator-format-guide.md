...
|---|---|---|
| **Claude (any version, generous context)** | `hypergraph.topothink.json` | Handles reified hyperedges and incidence lookups natively; preserves topology |
| **Gemma 3 / Gemma 4 (local via ollama)** | `normalized-dyadic.json` | Self-contained rows; no indirection; `layer` field lets the model filter by relationship type |
| **GPT-4 / GPT-4o (large context)** | `hypergraph.topothink.json` | Handles indirection well; equivalent to Claude on schema-novel reasoning |
...
### For Claude / GPT-4 / large-context models (hypergraph file)
...
### For Gemma / Llama / smaller models (dyadic file)
...
- **Chained pipelines**: it is fine (and often correct) to give Claude the hypergraph file, have it produce a structured intermediate, and then hand that intermediate to gemma for bulk processing. Different models for different layers of the same task.
