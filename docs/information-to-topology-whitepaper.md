...
The four tests are applied in a fixed precedence — transition, then enclosure, then propagation, with reference as the default — and the first test an edge passes fixes its primary category. This ordering is a deliberate design rule, not a claim that the four types are mutually exclusive in nature: an edge that exhibits more than one character (a punch both transmits force and changes state) takes the higher-precedence category as primary and records the other as a secondary tag. We name the precedence explicitly because the order itself does classificatory work that would otherwise remain silent.
